...
void uart_frame_init(int32_t rx, int32_t tx, int baud) {
  ...
  uart_base_init();
  xTaskCreatePinnedToCore(uart_frame_task, "uart_queue_task", 4 * 1024, NULL, 3, NULL, 1);
  // xTaskCreatePinnedToCore(uart_frame_send_task, "uart_frame_send_task", 2 * 1024, NULL, 1, NULL, 1);
}
              ...
              case kLenghtCheck:
                ...
                if (crc_xor == buf[packet_pos]) {
                  ...
                  if (frame_length > FRAME_MAX_SIZE - 9) {
                    Serial.println("==[failed] Acquired image size is larger than buffer.==");
                    packet_pos = 0;
